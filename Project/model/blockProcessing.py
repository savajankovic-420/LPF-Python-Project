...
def convolution(coeff, data, size, buffer):    
    filtered_data = np.zeros(size)
    output_buffer = []
    for i in range(size):
        if i < len(coeff) - 1:
            filtered_data[i] = np.dot(coeff[i:], buffer[:len(coeff)-1-i])
        else:
            filtered_data[i] = np.dot(coeff, data[i-len(coeff)+1:i+1])
            output_buffer.append(data[i-len(coeff)+1])
    return output_buffer, filtered_data

def blockFilter(audio_data, block_size, audio_Fc, audio_Fs, N_taps):
    coeff = lowpassfilter(audio_Fs, audio_Fc, N_taps)
    filtered_data = np.empty(shape = audio_data.shape)
    buffer = np.ones(len(coeff) - 1)

    for channel in range(audio_data.shape[1]):
        for i in range(0, audio_data.shape[0], block_size):
            filtered_data[i:i+block_size, channel], buffer = lfilter(coeff, 1, audio_data[i:i+block_size, channel], zi=buffer)
...
if __name__ == "__main__":
	...
	if (sys.argv[1] == 'rc'): # runs the reference code (rc)
		...
		block_processing_data = blockFilter(audio_data, \
							block_size = 1000, \
							audio_Fc = 10e3, \
							audio_Fs = audio_Fs, \
							N_taps = 51)
